...
def calculate_subtotal(order: list):
    ...
    if not isinstance(order, list) or len(order) < 1:
        print(f"type error {order = }")
        return total

    for _, menu_dict in enumerate(order):
        total += menu_dict['price']

    return round(total, 2)

def calculate_tax(subtotal: int | float) -> int | float:
    ...
    ### WRITE SOLUTION HERE
    if not (isinstance(subtotal, (int, float)) and subtotal >= 0.0):
        print(f"Type error {subtotal = } ")
        return 0.0

    return round(subtotal * 0.15, 2)

def summarize_order(order: list) -> tuple[list, float]:
    ...
    ### WRITE SOLUTION HERE
    items = []
    for _, item_dict in enumerate(order):
        name = item_dict['name']
        items.append(name)

    total = calculate_subtotal(order)

    return (items, round(total + calculate_tax(total), 2))
